...
$('#season-filter').innerHTML+=[...DATA.seasons].reverse().map(s=>`<option value="${esc(s.season_id)}">${esc(s.name)}</option>`).join('');
$('#stage-filter').innerHTML+=[...new Set(DATA.seasons.flatMap(s=>s.matches.map(m=>m.stageName)))].map(stage=>`<option>${esc(stage)}</option>`).join('');
function render(){const sid=$('#season-filter').value,stage=$('#stage-filter').value,q=$('#query').value.trim().toLowerCase();const seasons=[...DATA.seasons].reverse().filter(s=>!sid||s.season_id===sid);$('#list').innerHTML=seasons.map(s=>{const ms=s.matches.filter(m=>(!stage||m.stageName===stage)&&(!q||m.teamAName.toLowerCase().includes(q)||m.teamBName.toLowerCase().includes(q)));if(!ms.length)return'';const detail=s.champion?`冠军 <b>${esc(s.champion)}</b> · 总决赛 ${esc(s.final_team_a)} ${esc(s.final_score)} ${esc(s.final_team_b)}`:`${s.match_count} 场赛程`;return `<section class="season"><button class="season-head"><div class="season-title">${esc(s.name)}<i>${ms.length} 场⌄</i></div><div class="final">${detail}</div></button><div class="season-body">${ms.map(m=>`<div class="match"><span class="stage">${esc(m.stageName)}</span><span class="a">${esc(m.teamAName)}</span><span class="score">${m.teamAScore==null?'VS':esc(m.teamAScore)+':'+esc(m.teamBScore)}</span><span>${esc(m.teamBName)}</span></div>`).join('')}</div></section>`}).join('')||'<div class="empty">没有匹配的比赛</div>'}
$('#list').addEventListener('click',event=>{const head=event.target.closest('.season-head');if(head&&$('#list').contains(head))head.parentNode.classList.toggle('open')});
