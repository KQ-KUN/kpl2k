...
- **BGM 打断修复 + 三键联动 + 按钮置顶 + B站广告**（2026-08-21）：① 根因：bgm.js 候选音源全部 error 时把 `current` 置 null，导致同场景 play('battle') 误判为切换而重建音频（headless 实测 startCount 2→3）；修复为保持场景标记，`play()` 同场景仅 safePlay 恢复不重建（startCount 2→2 PASS）；② 三处音量键（首页/模拟页/结算页）统一 `syncBgmToggles()` 联动，任意开关同步更新所有图标，SVG 喇叭/斜杠图标（bgmIconHtml）；③ 首页开关移到右上角 fixed（.home-bgm-row #home-bgm-row）；④ 底部"民间算法·平行时空·非官方"删除，改为醒目的 B站广告按钮（.bili-ad：🎬 B站：几几华华里 ▶ 看视频），选手库 meta 同步清理。
