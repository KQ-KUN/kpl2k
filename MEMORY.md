...
- **确定化头像 + 年度战力单一口径**（2026-08-27）：`build_team_icons.py` 固定队伍 ID 顺序、Pillow 重采样、PNG 压缩参数与 JSON 序列化；`validate_build_consistency.py` 连续构建两次校验 SHA-256。选手战力在 `clean_kpl.py` 完成赛季校准后按“选手×年份”取正式战场赛事峰值（无有效正式评分时为 50），回填该年所有赛事；图鉴年度卡、赛事阵容和全明星版本因此复用同一战力，赛事统计/位置/效力队仍按实际记录。选手图鉴规则页已公开说明年度口径；发布门禁新增图鉴与赛事交叉校验。
